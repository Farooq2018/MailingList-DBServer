...
// Email Entry structure for database
...
// Function creates a Database structure from EmailEntry row
func emailEntryFromRows(row *sql.Rows) (*EmailEntry, error) {
	var id int64
	var email string
	...
	timeConv := time.Unix(confirmedAt, 0)
	return &EmailEntry{Id: id, Email: email, ConfirmedAt: &timeConv, OptOut: optOut}, nil
}
